...
const Profile = (Props) => {
  ...
  useEffect(() => {
    if (!Props.location.id) goTo('/index')
    console.log(Props)
    ...
    axios.get("http://127.0.0.1:8000/api/User/" + id).then((res) => {
      ...
      axios.post("http://127.0.0.1:8000/api/User/fav/" + id,{user_id:localStorage.getItem('user_id')})
        .then((res) => {
        console.log(res.data)
       setFav(res.data.fav)        
      })
      ...
      <div className="profileContainer">
        <div className="userInfo">
          <div className="uploadImg" >
            <img src={profileData.hasOwnProperty('user') ? profileData.user.img ? profileData.user.img : img : ""}
                          ...
                          <div className="select-items">
                            <div ClassName="Row">
                              <div>
                                ...
                                <input style={{textAlign:"center"}} className="addPostInput input2" defaultValue={profileData.user.name} placeholder={profileData.user.name} name="name" ref={register} />
                              </div>
                              <div>
                                <label className="label1 label2" > رقم الهاتف</label>
                                <input style={{textAlign:"center"}} type="number" className="addPostInput input2" defaultValue={profileData.user.phone_number}
                                  placeholder={profileData.user.phone_number} name="phone_number" ref={register} />
                              </div>
                              <div>
                                <label className="label1 label2" > الأيميل</label>
                                <input style={{textAlign:"center"}} className="addPostInput input2" placeholder={profileData.user.email} defaultValue={profileData.user.email} name="email" ref={register} />
                              </div>
                              <div>
                                <label className="label1 label2" > المدينة</label>
                                <select style={{textAlign:"center"}} name="location" className="input2" defaultValue={profileData.user.location} ref={register} >
                              ...
                              <div>
                                ...
                                {profileData.user.type === "admin" 
                                  ?
                                   <input  style={{textAlign:"center"}} className="addPostInput input2" placeholder="مسؤول" disabled />
                                  :
                                  <select name="type" style={{textAlign:"end"}} className="input2" defaultValue={profileData.user.type} ref={register} >
        ...
        <div className="userPostContainer">
          {postKind ? profileData.post.map((info) => <UserPosts
           key={info.id}  
          post_id={info.id}
          info={info}
           isLoged={Props.location.isLoged}
            ...
            type={'user'}
            />) :
            profileData.favorite.map((info) => <UserPosts
